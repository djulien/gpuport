...
//debug(wanted(30), detail());
    debug(`playing '${relpath(filename)}', duration ${wanted(50) && mp3len(filename) || "(TMI)"} sec ...`.yellow_lt); //CAUTION: mp3en() is expensive (reads entire file); don't display unless caller wants it
...
//                .on('progress', (data) => cb(`audio progress @T+${timestamp()}: ${JSON.stringify(data).json_tidy}`, null, data) || cancel()) //{numwr: THIS.numwr, wrlen: r, wrtotal: THIS.wrtotal, buflen: (left || []).length}); //give caller some progress info
                .on('open', () => cb(`audio open @T+${timestamp()}`) || cancel()) //pbstart = elapsed(); cb("audio open"); }) // /*pbtimer = setInterval(position, 1000)*/; console.log("[%s] speaker opened".yellow_light, timescale(pbstart - started)); })
...
//    inx || (inx = 0);
    debug(`playlist: start[${inx || 0}/${files.length}]`);
...
//        if (!data) console.log(evt, this, `time ${this.wrtotal / 4 / 44100}`);
        ...
        if (eof) setTimeout(() => playlist(files, opts, ++inx || 1), opts.gap || 500); //give a little time in between
...
//set default options:
    const opts =
    {
        ...
        get pattern() { return this.folder + "/*.mp3"; }, //"*.mp3"
//    seqname: "*.seq",
        autoplay: true,
        repeat: 2,
        gap: 0.5, //sec between songs
//    async: false,
    };
...
//map single unnamed arg to named options:
    let files = process.argv
        .slice(2) //drop node + my source file name
        .map((arg) =>
        {
            if (!arg.match(/^[+\-]/)) return arg; //use filename as-is
            opts[arg] = (arg[0] == "+"); //set true/false option
//TODO: string/numeric options
    ...
    if (!files.length) files.push.apply(files, glob.sync(opts.pattern)); //pattern, {}); //, function (err, files)
