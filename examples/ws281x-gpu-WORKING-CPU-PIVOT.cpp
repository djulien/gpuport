...
// create an EGL window surface, passing context width/height
	...
	if (graphics_get_display_size(0 /* LCD */, &display_width, &display_height) < 0) return why(FALSE, "!display");
...
//printf("egl: disp 0x%x\n", display);
	if (state.eglDisplay == EGL_NO_DISPLAY) return why(FALSE, "!egl disp");

//	EGLConfig config;
	EGLint numConfigs;
    EGLConfig cfgs[30];
	EGLint majorVersion, minorVersion;
	if (!eglInitialize(state.eglDisplay, &majorVersion, &minorVersion)) return why(FALSE, "!egl init");
...
//setEGLConfigChooser(8, 8, 8, 8, 16, 0) 
	if (!eglChooseConfig(state.eglDisplay, attribList, cfgs, LEN(cfgs), &numConfigs) || !numConfigs) return why(FALSE, "!egl choose cfg");
...
//    if (!render || !bind_rgb || !bind_rgba || !depth) continue;
    ...
    if (found < 0) return why(FALSE, "!suitable egl config found");
...
//RPi was selecting a bad config without the above loop to explicitly step thru all configs!  don't just use cfg[0]
	...
	if (state.eglSurface == EGL_NO_SURFACE) return why(FALSE, "!surf");
	/*EGLContext*/ state.eglContext = eglCreateContext(state.eglDisplay, cfgs[found], EGL_NO_CONTEXT, contextAttribs);
	if (state.eglContext == EGL_NO_CONTEXT) return why(FALSE, "!cre ctx");
	if (!eglMakeCurrent(state.eglDisplay, state.eglSurface, state.eglSurface, state.eglContext)) return why(FALSE, "!cur disp");
...
//    "   else if (ISDEBUG) outcolor = texture2D(s_texture, trueimg * OVERSCAN);\n" //original rendering; (S, T) swizzle
//    "   else if (!ENABLED) outcolor = texture2D(s_texture, pivotimg * OVERSCAN);\n" //original rendering; (S, T) swizzle
...
//    "   return true;\n"
    ...
    "   if (bit < 16.0) return false; //LSB(255.0 * bits.g / pow(2.0, 15.0 - bit));\n"
    "   if (bit < 24.0) return false; //LSB(255.0 * bits.b / pow(2.0, 23.0 - bit));\n"
    "   return true;\n" //bug: should not happen
    "}\n"
...
//    "   if (mask < 256.0) return bits + vec4(mask / 256.0, 0.0, 0.0, 0.0);\n"
    ...
    "   else if (bit < 8.0) pow(0.5, bit + 1.0);\n"
    "   else if (bit < 16.0) pow(0.5, bit - 7.0);\n"
    "   else if (bit < 24.0) pow(0.5, bit - 15.0);\n"
    "   else bits = ERROR(1);\n" //bug: should not happen
    "   return bits;\n"
    "}\n"
    "bool LSB(float val)\n"
